...
if id>1 % if dot 1 enters "trigger zone"
    ...
    id2 = id1+floor(0.05*fs);
    C_x1(id1+1 : id2) = b/0.05*cos(c*pi/180)*(1/fs:1/fs:0.05);
    C_y1(id1+1 : id2) = b/0.05*sin(c*pi/180)*(1/fs:1/fs:0.05);
    
    % Phase 2 can be disturbed by dot 2
    id22 = id2 + floor(0.375*fs);
    C_x2(id2+1 : id22) = C_x1(id2) - b/0.375*cos(c*pi/180)*(1/fs:1/fs:0.375);
    C_y2(id2+1 : id22) = C_y1(id2) - b/0.375*sin(c*pi/180)*(1/fs:1/fs:0.375);
    ...
    if id3>1 %if dot 2 triggers movement
        ...
        % Phase1
        C_x1(id4+1:id5) = C_x2(id4) + b/0.05*cos(c*pi/180)*(1/fs:1/fs:0.05);
        C_y1(id4+1:id5) = C_y2(id4) + b/0.05*sin(c*pi/180)*(1/fs:1/fs:0.05);
        
        % Phase 2
        tb = C_x1(id5)/(b/0.375*cos(c*pi/180)); % time needed to return to original position 
        tb_id = floor(tb*fs);
        
        C_x1(id5+1:id5+tb_id) = C_x1(id5) - b/0.375*cos(c*pi/180)*(1/fs:1/fs:tb);
        C_y1(id5+1:id5+tb_id) = C_y1(id5) - b/0.375*sin(c*pi/180)*(1/fs:1/fs:tb);
